<?php
    ...
    if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password) && !empty($branch)) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if (mysqli_num_rows($sql) > 0) {
                ...
                $insert_query = mysqli_query($conn, "INSERT INTO users (unique_id, fname, lname, email, password, img, status, branch)
                                VALUES ({$ran_id}, '{$fname}','{$lname}', '{$email}', '{$encrypt_pass}', '{$new_img_name}', '{$status}','{$branch}')");
                if ($insert_query) {
